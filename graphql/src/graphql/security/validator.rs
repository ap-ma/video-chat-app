...
pub fn password_validator(
    ...
    const PATTERN: &str = r"^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[a-zA-Z0-9]{8,24}$";
    if let Ok(true) = Regex::new(PATTERN).map(|re| re.is_match(value).unwrap_or(false)) {
...
pub fn code_validator(
    ...
    // 最低4文字 最大8文字 半角英数字
    const PATTERN: &str = r"^[a-zA-Z0-9]{4,8}$";
    if let Ok(true) = Regex::new(PATTERN).map(|re| re.is_match(value).unwrap_or(false)) {
